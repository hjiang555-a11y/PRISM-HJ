...
2. **`applicability_conditions` 动态推断（✅ 已完成，见第十节）**：当前 mapper 中的 `assumptions`
   已可根据 `assumption_hints` 动态区分来源（语义层标注 vs. 原型默认 fallback）。
   完整的 `applicability_conditions` 动态推断已在 admission 闭环增强轮次落地，
   新增 `applicability_eval` 字段提供结构化动态评估结果。
...
4. **`validity_limits` 轻量校验机制（✅ 已完成，见第十节）**：已实现 `validity_warnings` 结构化警告机制；
   当文本中检测到可能违反 validity_limits 的信号时，mapper 生成结构化警告条目，
   不影响 admission 三态，但可被测试断言。

5. **deferred capability 的重入机制（✅ 最小骨架已完成，见第十节）**：已在 `deferred_capabilities` 条目中
   增加 `reentry_hints` 结构化重入建议，并提供 `build_reentry_context()` 重入接口。
   完整的交互式重入循环（用户或 LLM 补充后自动重新 admission）留待后续轮次。
...
本轮（P0 第四步）完成后，以下 admission 判断已有结构化语义来源（不再只依赖关键词）：
...
以下内容仍保持 B 层关键词匹配（语义层暂无更好来源，留待 admission 闭环增强轮次解决）：
- `initial_position_per_entity` 的缺失判断
- `initial_velocity_per_entity` 的缺失判断
- `mass_per_entity` 的缺失判断

✅ **上述三项在 admission 闭环增强轮次中通过新增 `InputAvailabilityHints` 结构化来源得到改善（见第十节）。**

---

## 十、Admission 闭环增强（接续 P0 第四步）

本节记录在 P0 第四步之后，继续推进的四项 admission 尾项收口工作。

### 10.1 输入已知性的结构化语义来源（Goal 1）

**新增 `InputAvailabilityHints` 模型**（`src/problem_semantic/models.py`）：

```python
class InputAvailabilityHints(BaseModel):
    initial_position_known: bool = False
    initial_velocity_known: bool = False
    mass_known: bool = False
    pre_collision_velocity_known: bool = False
    sources: Dict[str, str] = {}  # 各字段信息来源
```

**extraction pipeline 扩展**（`src/problem_semantic/extraction/pipeline.py`）：

新增 `_extract_input_availability_hints(text)` 函数，通过正则表达式从文本中推断各物理量是否可得：
- 策略 `text_explicit`：文本中出现量纲词紧跟数值表达式
- 策略 `text_inferred`：文本中出现量纲词或单位但无明显数值
- 策略 `default`：未检测到相关量纲词，默认不可得（False）

**mapper 更新**：

`particle_motion` 和 `contact_interaction` mapper 的 A 层判断现在优先读取 `input_availability_hints`：

```python
# 之前（仅 B 层）：
_has_velocity_from_semantics = False

# 之后（A 层来自 input_availability_hints）：
_has_velocity_from_semantics = problem_spec.input_availability_hints.initial_velocity_known
```

A 层缺失时，B 层（`explicit_conditions` 关键词匹配）仍正常 fallback。

### 10.2 applicability_conditions 动态评估（Goal 2）

**新增 `ApplicabilityEvalItem` 模型**（`src/capabilities/common/base.py`）：

```python
class ApplicabilityEvalItem(BaseModel):
    condition_key: str        # 条件标识键（机器可读）
    description: str          # 条件描述
    status: str               # "satisfied" | "uncertain" | "unsupported"
    source: str               # 评估依据来源
    notes: Optional[str]      # 补充说明
```

**CapabilitySpec 新增字段** `applicability_eval: List[ApplicabilityEvalItem]`，
与现有 `applicability_conditions`（静态文本列表）**并存**，不替换。

**mapper 动态评估逻辑**：

*ParticleMotionCapabilitySpec（共 4 条评估项）*：
- `point_mass_applicable`：依据 `entity_model_hints`，`rigid_body` → unsupported，`point_mass` → satisfied，无 hint → uncertain
- `continuous_background_action`：依据 `interaction_hints`，`gravity_present` → satisfied，`field_present` → uncertain，无 → uncertain
- `continuous_state_evolution`：默认 satisfied（粒子运动的基础假设）
- `no_local_trigger_interrupt`：依据 `interaction_hints`，`collision_possible` → uncertain，否则 → satisfied

*ContactInteractionCapabilitySpec（共 4 条评估项）*：
- `at_least_two_entities`：依据实体列表长度，≥2 → satisfied，=1 → unsupported，=0 → uncertain
- `collision_event_identified`：依据 `interaction_hints`，`collision_possible` 或 `contact_possible` → satisfied，否则 → uncertain
- `instantaneous_impulse_approximation`：仅有 `contact_possible` 无 `collision_possible` → uncertain，有 `collision_possible` → satisfied
- `pre_trigger_state_available`：依据 `pre_trigger` 和 `input_availability_hints`，有信息 → satisfied，否则 → uncertain

### 10.3 deferred capability 最小重入骨架（Goal 3）

**新增 `build_reentry_context()` 函数**（`src/planning/execution_plan/builder.py`）：

为 deferred capability 生成结构化重入上下文，包含：
- `supplemental_suggestions`：逐项补充建议（含 label、suggestion、example_question、entity_context）
- `reentry_note`：重入说明，提示"补充入口要素后可重新调用 `build_execution_plan()`"

**ExecutionPlan 中的变化**：

`deferred_capabilities` 中每条记录新增 `reentry_hints` 字段：
...
```python
{
    ...
    "reentry_hints": {
        ...
        "reentry_note": "capability 'particle_motion' 当前处于 deferred 状态。...",
    }
}
```

**已实现的最小版本**：结构化补充建议 + 重入接口可调用。

**剩余尾项**：完整的交互式重入循环（用户或 LLM 补充后自动触发重新 admission）。

### 10.4 validity_limits 轻量 warning 机制（Goal 4）

**新增 `ValidityWarning` 模型**（`src/capabilities/common/base.py`）：

```python
class ValidityWarning(BaseModel):
    warning_key: str       # 警告标识键（机器可读）
    description: str       # 警告描述
    triggered_by: str      # 触发该警告的文本线索
```

**CapabilitySpec 新增字段** `validity_warnings: List[ValidityWarning]`。

**mapper 警告检测逻辑**：

*ParticleMotionCapabilitySpec（2 类警告）*：
- `rotation_hint_in_point_mass`：文本中出现旋转/角速度相关关键词
- `complex_field_in_simple_background`：文本中出现非均匀场/变化重力相关关键词

*ContactInteractionCapabilitySpec（2 类警告）*：
- `continuous_contact_in_impulse_model`：文本中出现持续接触/滑动/摩擦相关关键词
- `multi_body_collision`：实体数量超过 2 或文本中出现多体碰撞关键词

**重要约束**：validity_warnings 不改变 admission 三态（admitted/deferred/unresolved），
仅提供可被测试断言的结构化信号。
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 四类 admission hints（P0 第四步） | ✅ 已完成 | entity_model / interaction / assumption / query |
| 三层优先级（A/B/C）信息消费 | ✅ 已完成 | mapper 已按 A→B→C 优先级消费信息 |
| `InputAvailabilityHints` 结构化输入可得性 | ✅ 已完成 | 速度、质量、位置、碰前速度的 A 层语义来源 |
| `applicability_eval` 动态评估 | ✅ 已完成（最小版本） | 4 条评估项（particle_motion）、4 条（contact_interaction） |
| `validity_warnings` 轻量 warning | ✅ 已完成（最小版本） | 2 类（particle_motion）、2 类（contact_interaction） |
| deferred 重入骨架 | ✅ 最小骨架已完成 | `build_reentry_context()` + `reentry_hints` |
| 完整交互式重入循环 | 🔲 待后续 | 补充后自动重新 admission |
| 更多 capability 准入对齐 | 🔲 待后续 | 新增 capability 按约定格式填写准入字段 |
